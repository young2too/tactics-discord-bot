...
export class SingleRoom {
  ...
  resolve(actor, skillId, target, guessedRole, text) {
    ...
    if (["ally-scan", "enemy-scan", "advanced-scan"].includes(skillId)) { this.effect = { id: target.id, type: "inspect", until: this.now() + 1500 }; this.recordInspection(target); const hit = guessedRole === target.role; const message = hit ? `${target.nickname}은(는) ${target.role}입니다.` : `${target.nickname}은(는) ${guessedRole}이(가) 아닙니다.`; this.addLog("🔎", `누군가가 ${target.nickname}을(를) 살피고 있습니다.`, "scan"); this.private(actor, `스캔 ${hit ? "성공" : "실패"} · ${message}`); this.verdict(actor, "스캔 판정", hit, message); return; }
    if (["ally-check", "enemy-check"].includes(skillId)) { this.effect = { id: target.id, type: "inspect", until: this.now() + 1500 }; this.recordInspection(target); const fooled = skillId === "ally-check" && target.role === "스파이" && factionOf(target.announced) === "citizen" && factionOf(actor.role) === "citizen"; const hit = fooled || target.announced === target.role; const message = `${target.nickname}의 ${target.announced} 공표는 ${hit ? "진명" : "가명"}입니다.`; this.addLog("🔎", `누군가가 ${target.nickname}을(를) 살피고 있습니다.`, "scan"); this.private(actor, message); this.verdict(actor, "공표 확인", hit, message); return; }
    if (skillId === "boss-check" || skillId === "detective-check") { this.effect = { id: target.id, type: "inspect", until: this.now() + 1500 }; this.recordInspection(target); const role = skillId === "boss-check" ? "마피아대부" : "사립탐정"; const hit = target.role === role; const message = `${target.nickname}은(는) ${role}${hit ? "이 맞습니다" : "이 아닙니다"}.`; this.private(actor, message); this.verdict(actor, "직업 확인", hit, message); return; }
  ...
  chat(playerId, payload) {
    ...
    if (whisper) { const target = this.player(Number(whisper[1])); const body = whisper[2].trim(); const sentAt = this.now(); const message = { from: actor.id, to: target.id, text: body, until: sentAt + 5500 }; actor.whisper = message; target.whisper = message; this.private(actor, `${target.id}번 ${target.nickname}에게 귓말 · ${body}`); this.private(target, `${actor.id}번 ${actor.nickname}의 귓말 · ${body}`); if (!actor.aiControlled && target.aiControlled) { target.aiMemory ??= {}; target.aiMemory.inbox ??= []; target.aiMemory.inbox.push({ from: actor.id, text: body, channel: "whisper", at: sentAt }); this.nextBotAt = Math.min(this.nextBotAt ?? Infinity, sentAt + 650); } return; }
    const allianceText = text.match(/^\/a\s+(.+)$/s)?.[1] ?? (payload.channel === "alliance" ? text : null);
    if (allianceText) { if (!actor.alliances.size) throw new Error("동맹이 없습니다."); const sentAt = this.now(); this.chats.push({ id: sentAt, from: actor.id, text: allianceText, channel: "alliance", recipients: [actor.id, ...actor.alliances] }); if (!actor.aiControlled) { const listeners = this.players.filter((player) => actor.alliances.has(player.id) && player.alive && player.aiControlled); for (const listener of listeners) { listener.aiMemory ??= {}; listener.aiMemory.inbox ??= []; listener.aiMemory.inbox.push({ from: actor.id, text: allianceText, channel: "alliance", at: sentAt }); } if (listeners.length) this.nextBotAt = Math.min(this.nextBotAt ?? Infinity, sentAt + 650); } }
    else { const sentAt = this.now(); this.chats.push({ id: sentAt, from: actor.id, text, channel: "public" }); const relayToAi = !actor.aiControlled || payload.aiBroadcast === true; const listeners = relayToAi ? this.players.filter((player) => player.alive && player.aiControlled && player.id !== actor.id) : []; listeners.forEach((listener, index) => { listener.aiMemory ??= {}; listener.aiMemory.inbox ??= []; listener.aiMemory.inbox.push({ from: actor.id, text, channel: "public", at: sentAt, respond: !actor.aiControlled && index === 0 }); }); if (listeners.length) this.nextBotAt = Math.min(this.nextBotAt ?? Infinity, sentAt + 900); }
