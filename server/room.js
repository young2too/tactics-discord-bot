...
export class SingleRoom {
  ...
  resolve(actor, skillId, target, guessedRole, text) {
    ...
    if (skillId === "proclamation") { if (!text) throw new Error("공문 내용을 입력하세요."); const message = "공문을 전체 플레이어에게 발송했습니다."; this.chat(actor.id, { text: `[공문] ${text}`, aiBroadcast: true }); this.addLog("📢", `공무원 공문 · ${text}`, "plain"); this.private(actor, message); this.verdict(actor, "공문 발송", true, message); return; }
