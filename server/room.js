...
export class SingleRoom {
  ...
  resolve(actor, skillId, target, guessedRole, text) {
    ...
    if (skillId === "tail") { const until = this.now() + 30_000; this.tails = this.tails.filter((entry) => entry.spyId !== actor.id && entry.until > this.now()); this.tails.push({ spyId: actor.id, targetId: target.id, until }); const message = `${target.id}번 ${target.nickname}을(를) 30초 동안 미행합니다.`; this.private(actor, message); this.verdict(actor, "미행 시작", true, message); this.private(target, "누군가 당신을 지켜보고 있습니다."); this.notify(target, "수상한 시선", "누군가 당신을 지켜보고 있습니다.", "hostile"); return; }
    ...
    if (skillId === "successor") { const hit = target.role === "마피아후계자"; const message = hit ? `${target.nickname}을(를) 후계자로 지정했습니다.` : `${target.nickname}은(는) 마피아후계자가 아닙니다.`; if (hit) { this.successorId = target.id; this.private(target, `${actor.id}번 ${actor.nickname}이(가) 마피아대부로서 당신을 후계자로 지정했습니다.`); this.notify(target, "후계자 임명", `${actor.id}번 ${actor.nickname}이(가) 당신을 후계자로 지정했습니다.`, "alliance"); if (target.aiControlled) { target.aiMemory ??= {}; target.aiMemory.knowledge ??= {}; target.aiMemory.trust ??= {}; target.aiMemory.claims ??= {}; target.aiMemory.knowledge[actor.id] = { role: "마피아대부", faction: "mafia", confidence: 1, source: "후계자 임명", excluded: [] }; target.aiMemory.trust[actor.id] = 1; target.aiMemory.claims[actor.id] = { role: "마피아대부", trust: 1, source: "후계자 임명" }; this.wakeAi(target); } } this.private(actor, message); this.verdict(actor, "후계자 지정", hit, message); this.addLog("♛", "마피아대부가 후계자를 지정했습니다.", "plain"); return; }
