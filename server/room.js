...
export class SingleRoom {
  ...
  start(playerId) {
    ...
    shuffle(this.players, this.random).forEach((player, index) => { player.role = roles[index]; });
    ...
    if (maleLover && femaleLover) {
      this.notify(maleLover, "연인 정보", `${femaleLover.id}번 ${femaleLover.nickname}은(는) 여자연인입니다.`, "alliance");
      this.notify(femaleLover, "연인 정보", `${maleLover.id}번 ${maleLover.nickname}은(는) 남자연인입니다.`, "alliance");
  ...
  act(playerId, payload) {
    ...
    this.revealFollowedAction(actor, skillId, target, guessedRole);
  ...
  validateSpecial(actor, skillId, target) {
    ...
    if (skillId === "betrayal" && (!actor.incomingAlliances.has(target.id) || !target.incomingAlliances.has(actor.id))) throw new Error("배신은 서로 동맹을 보낸 맞동맹에게만 사용할 수 있습니다.");
  }
  resolve(actor, skillId, target, guessedRole, text) {
    if (skillId === "announce") { actor.announced = guessedRole; const gain = guessedRole === actor.role ? 10 : 5; actor.mana = Math.min(MANA_MAX, actor.mana + gain); const message = `${guessedRole} 공표 완료`; this.addLog("📜", `${actor.nickname}이(가) ${guessedRole}(을)를 공표했습니다.`, "plain"); this.private(actor, message); this.verdict(actor, "공표 완료", true, `${message} · 마나 +${gain}`); return; }
    ...
    if (skillId === "tail") { const until = this.now() + 30_000; this.tails = this.tails.filter((entry) => entry.spyId !== actor.id && entry.until > this.now()); this.tails.push({ spyId: actor.id, targetId: target.id, until }); const message = `${target.id}번 ${target.nickname}을(를) 30초 동안 미행합니다.`; this.private(actor, message); this.verdict(actor, "미행 시작", true, message); this.notify(target, "수상한 시선", "누군가 당신을 지켜보고 있습니다.", "hostile"); return; }
    ...
    if (skillId === "ally-add" || skillId === "ally-remove") {
      ...
      if (adding) syncAllianceIntel(this, actor, target);
      return;
    }
    ...
    if (skillId === "support") { const message = `${target.nickname}에게 마나 30을 지원했습니다.`; target.mana = Math.min(MANA_MAX, target.mana + 30); this.addLog("+", `공무원이 ${target.nickname}에게 마나를 지원합니다.`, "mana"); this.private(actor, message); this.notify(target, "마나 지원", "공무원에게서 마나 30을 지원받았습니다.", "alliance"); this.verdict(actor, "지원 완료", true, message); return; }
    if (skillId === "betrayal") { this.effect = { id: target.id, type: "attack", until: this.now() + 1500 }; const message = `${target.nickname}을(를) 배신하여 즉시 처치했습니다.`; this.kill(target, "배신"); this.private(actor, message); this.verdict(actor, "배신 성공", true, message); return; }
    if (skillId === "successor") { const hit = target.role === "마피아후계자"; const message = hit ? `${target.nickname}을(를) 후계자로 지정했습니다.` : `${target.nickname}은(는) 마피아후계자가 아닙니다.`; if (hit) { this.successorId = target.id; this.notify(target, "후계자 임명", `${actor.id}번 ${actor.nickname}이(가) 당신을 후계자로 지정했습니다.`, "alliance"); if (target.aiControlled) { target.aiMemory ??= {}; target.aiMemory.knowledge ??= {}; target.aiMemory.trust ??= {}; target.aiMemory.claims ??= {}; target.aiMemory.knowledge[actor.id] = { role: "마피아대부", faction: "mafia", confidence: 1, source: "후계자 임명", excluded: [] }; target.aiMemory.trust[actor.id] = 1; target.aiMemory.claims[actor.id] = { role: "마피아대부", trust: 1, source: "후계자 임명" }; this.wakeAi(target); } } this.private(actor, message); this.verdict(actor, "후계자 지정", hit, message); this.addLog("♛", "마피아대부가 후계자를 지정했습니다.", "plain"); return; }
    if (skillId === "snipe-command") {
      const hit = target.role === "히트맨";
      if (hit) { target.snipeAuthorized = true; target.snipeCommanderId = actor.id; this.private(actor, `${target.nickname}에게 저격명령을 내렸습니다.`); this.notify(target, "저격명령 활성화", "마피아대부의 저격명령을 받았습니다. 저격 1회가 활성화됩니다.", "alliance"); }
  ...
  chat(playerId, payload) {
    ...
    if (whisper) { const target = this.player(Number(whisper[1])); const body = whisper[2].trim(); const sentAt = this.now(); const message = { from: actor.id, to: target.id, text: body, until: sentAt + 5500 }; actor.whisper = message; target.whisper = message; if (!actor.aiControlled && target.aiControlled) { target.aiMemory ??= {}; target.aiMemory.inbox ??= []; const incoming = { id: sentAt, from: actor.id, text: body, channel: "whisper", at: sentAt }; target.aiMemory.inbox.push(incoming); this.enrichAiMessage(actor, [target], incoming); this.wakeAi(target, sentAt); } return; }
    const allianceText = text.match(/^\/a\s+(.+)$/s)?.[1] ?? (payload.channel === "alliance" ? text : null);
    if (allianceText) { const recipients = this.players.filter((player) => player.alive && player.incomingAlliances.has(actor.id)); if (!recipients.length) throw new Error("내가 동맹을 보낸 생존자가 없습니다."); const sentAt = this.now(); this.chats.push({ id: sentAt, from: actor.id, text: allianceText, channel: "alliance", recipients: [actor.id, ...recipients.map((player) => player.id)] }); if (!actor.aiControlled) { const listeners = recipients.filter((player) => player.aiControlled); const incomingMessages = []; for (const listener of listeners) { listener.aiMemory ??= {}; listener.aiMemory.inbox ??= []; const incoming = { id: sentAt, from: actor.id, text: allianceText, channel: "alliance", at: sentAt }; listener.aiMemory.inbox.push(incoming); incomingMessages.push(incoming); this.wakeAi(listener, sentAt); } if (listeners.length) this.enrichAiMessage(actor, listeners, incomingMessages); } }
  ...
  revealFollowedAction(actor, skillId, target, guessedRole) { const ignored = new Set(["announce", "ally-add", "ally-remove", "deception", "tail", "betrayal", "proclamation"]); if (ignored.has(skillId)) return; const current = this.now(); const labels = { "snipe-command": "저격명령", successor: "후계자 지정", leadership: "리더십", snipe: "저격", revenge: "복수귀", arrest: "검거", support: "지원", "boss-check": "보스 확인", "detective-check": "탐정 확인", "ally-check": "아군 확인", "enemy-check": "적군 확인", "advanced-scan": "상급 스캔", "enemy-scan": "적군 스캔", "upper-attack": "상급공격", "lower-attack": "하급공격" }; const uniqueRoles = { "snipe-command": "마피아대부", successor: "마피아대부", snipe: "히트맨", arrest: "경찰반장", support: "공무원", "boss-check": "마피아후계자", "advanced-scan": "마피아후계자", "detective-check": "탐정조수" }; this.tails = this.tails.filter((entry) => entry.until > current && this.players.some((player) => player.id === entry.spyId && player.alive)); for (const entry of this.tails.filter((item) => item.targetId === actor.id)) { const spy = this.players.find((player) => player.id === entry.spyId); if (!spy?.alive) continue; const targetText = target ? ` · 대상 ${target.id}번 ${target.nickname}` : ""; const roleText = guessedRole ? ` · 지목 ${guessedRole}` : ""; const message = `미행 정보 · ${actor.id}번 ${actor.nickname}이(가) ${labels[skillId] ?? skillId}을(를) 사용했습니다${targetText}${roleText}.`; this.private(spy, message); this.notify(spy, "미행 정보", message, "alliance"); const inferredRole = uniqueRoles[skillId]; if (spy.aiControlled && inferredRole) { spy.aiMemory ??= {}; spy.aiMemory.knowledge ??= {}; spy.aiMemory.trust ??= {}; spy.aiMemory.knowledge[actor.id] = { role: inferredRole, faction: factionOf(inferredRole), confidence: 1, source: `미행으로 ${labels[skillId]} 포착`, excluded: [] }; } } }
