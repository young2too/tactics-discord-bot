...
export class SingleRoom {
  ...
  chat(playerId, payload) {
    ...
    if (allianceText) { const recipients = this.players.filter((player) => player.alive && player.incomingAlliances.has(actor.id)); if (!recipients.length) throw new Error("내가 동맹을 보낸 생존자가 없습니다."); const sentAt = this.now(); this.chats.push({ id: sentAt, from: actor.id, text: allianceText, channel: "alliance", recipients: [actor.id, ...recipients.map((player) => player.id)] }); if (!actor.aiControlled) { const listeners = recipients.filter((player) => player.aiControlled); const incomingMessages = []; for (const [index, listener] of listeners.entries()) { listener.aiMemory ??= {}; listener.aiMemory.inbox ??= []; const incoming = { id: sentAt, from: actor.id, text: allianceText, channel: "alliance", at: sentAt, respond: index === 0 }; listener.aiMemory.inbox.push(incoming); incomingMessages.push(incoming); this.wakeAi(listener, sentAt); } if (listeners.length) this.enrichAiMessage(actor, listeners, incomingMessages); } }
