...
export class SingleRoom {
  ...
  resolve(actor, skillId, target, guessedRole, text) {
    ...
    if (skillId === "arrest") { this.effect = { id: target.id, type: "attack", until: this.now() + 1500 }; if (target.role !== "마피아대부") this.result = { winner: "mafia", reason: "경찰반장의 검거 실패" }; else { this.kill(target, "검거"); const successorAlive = this.successorId && this.player(this.successorId).alive; if (!successorAlive) this.result = { winner: "citizen", reason: "마피아대부 검거 성공" }; } return; }
    if (["snipe", "revenge"].includes(skillId)) { this.effect = { id: target.id, type: "attack", until: this.now() + 1500 }; if (skillId === "snipe") actor.snipeAuthorized = false; this.kill(target, skillId === "snipe" ? "저격" : "복수"); return; }
