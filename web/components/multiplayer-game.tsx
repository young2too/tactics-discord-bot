...
export function MultiplayerGame({ room }: { room: RoomController }) {
  ...
  return <main className={`game-shell size-${players.length}`}>
    ...
    <section className="battle-layout">
      <CommunicationPanel players={players} messages={(state.chats ?? []) as ChatMessage[]} channel={chatChannel} setChannel={setChatChannel} input={chatInput} setInput={setChatInput} onSubmit={sendChat} logs={state.logs ?? []} />
      ...
      <aside className="detail-panel panel"><div className="panel-heading"><span>전술 정보</span><b>PRIVATE</b></div><div className="role-card"><span className="role-kicker">나의 실제 직업</span><div className="role-portrait-large"><span style={portraitStyle(me.role)} /></div><h2>{me.role}</h2><span className="faction-tag">{me.faction === "mafia" ? "마피아" : "시민"} 진영</span></div><div className="private-result"><span>개인 판정</span><div className="private-log-list">{(state.privateLogs ?? []).map((log, index) => <p key={`${log.time}-${index}`}><time>{log.time}</time>{log.text}</p>)}</div></div><div className="mana-block"><div><span>마나</span><strong>{state.mana ?? 0}<small> · 보급 {manaTick}</small></strong></div><div className="mana-track"><i style={{ width: `${Math.min(100, (state.mana ?? 0) / 2)}%` }} /></div></div></aside>
    </section>
    <SkillDeck me={me} notice={notice} skills={skills} selectedSkill={selectedSkill} cooldowns={state.cooldowns ?? {}} usedOnce={state.usedOnce ?? {}} gameResult={state.result ?? null} onChoose={chooseSkill} />
