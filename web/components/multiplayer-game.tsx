...
export function MultiplayerGame({ room }: { room: RoomController }) {
  ...
  return <main className={`game-shell size-${players.length} mobile-panel-${mobilePanel ?? "closed"}`}>
    ...
    <section className="battle-layout">
      ...
      <Battlefield players={players} mySeatIndex={mySeatIndex} selectedSkill={selectedSkill} notice={notice} effectTarget={state.effect ?? null} alliances={state.alliances ?? []} whisper={state.whisper ?? null} onCancel={cancel} onTarget={chooseTarget} onWhisperPrefill={(player) => { setChatChannel("public"); setChatInput(`-${player.id} `); setMobilePanel("chat"); }} isTargetable={isTargetable} />
    ...
    {state.verdict && <div className={`verdict-popup ${state.verdict.success ? "success" : "failure"}`} key={state.verdict.id} role="status"><span>{state.verdict.title}</span><strong>{state.verdict.success ? "성공" : "실패"}</strong><p>{state.verdict.message}</p></div>}
