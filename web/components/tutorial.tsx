...
const basicSteps = [
  ...
  ["마나 보급을 확인하세요", "마나는 1분 30초마다 기본 20이 들어옵니다. 나에게 동맹을 보낸 생존자 한 명마다 10이 추가됩니다. 민준이 맞동맹을 보낸 상황에서 보급을 받아보세요."],
...
function BasicTraining({ onComplete, onExit }: { onComplete: () => void; onExit: () => void }) {
  ...
  const [incomingAllianceCount, setIncomingAllianceCount] = useState(0);
  const me = players[0];
  ...
  const expectedSkill = [null, "announce", null, null, "ally-add", null, null, "ally-check", "lower-attack", "lower-attack"][step];
  const calloutSelector = step === 0 ? ".board-area"
    ...
    : step === 7 ? (selectedSkill ? ".player-seat:nth-of-type(4)" : ".skill-ally-check")
    : step === 8 ? (selectedTarget ? '.role-choice[data-role="히트맨"]' : selectedSkill ? ".player-seat:nth-of-type(3)" : ".skill-lower-attack")
  ...
  useEffect(() => {
    ...
    else if ([1, 4, 7, 8, 9].includes(step)) setMobilePanel("skills");
    else if (step === 6) setMobilePanel("info");
  ...
  useEffect(() => { if (step === 6) setIncomingAllianceCount(1); }, [step]);
  useEffect(() => {
